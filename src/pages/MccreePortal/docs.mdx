import { Anchor, Center, Image, Blockquote } from '@mantine/core';
...
`renderer.toneMapping = ACESFilmicToneMapping;`是用`toneMapping`时会发现`portal`内的颜色和主场景不同, 使用默认的`NoToneMapping`
即可, 可是什么是`ToneMapping`呢?

<Blockquote icon={<IconInfoCircle />} mb='xl'>
